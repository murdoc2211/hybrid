...
summ=pd.DataFrame([
 ...
 ['',''],
 ['-- how we get there --',''],
 ['Amazon open PO in scope (NOT a recommendation)',int(df.open_po.sum())],
 ['  less: unsupplyable (EOL or out of stock)',-int(df[df.soldout].open_po.sum())],
 ['  less: held back - would breach 35d cover',-int(df[~df.soldout].hold.sum())],
 ['  = SHIP NOW',int(df.ship.sum())],
 ['  ship as % of open PO','%.0f%%'%(df.ship.sum()/df.open_po.sum()*100)],
 ['',''],
 ['-- overstock check --',''],
 ['Established SKUs shipped',int(len(df[(df.ship>0)&(df.drr_sep>0)&(df.arrival=='')]))],
 ['Max DOH of any established SKU after ship','%.0f'%df[(df.ship>0)&(df.drr_sep>0)&(df.arrival=='')].doh_after.max()],
 ['Account cover now (days)','%.0f'%(df.sellable.sum()/df.drr_sep.sum())],
 ['Account cover after this shipment (days)','%.0f'%((df.sellable.sum()+df.ship.sum())/df.drr_sep.sum())],
 ['Account cover IF we shipped the full open PO','%.0f'%((df.sellable.sum()+df.open_po.sum())/df.drr_sep.sum())],
 ['Inherited excess above 35d we ship nothing to',int((df[(df.drr_sep>0)&(df.doh_now>35)&(df.ship==0)].sellable-df[(df.drr_sep>0)&(df.doh_now>35)&(df.ship==0)].drr_sep*35).clip(lower=0).sum())],
 ['',''],
 ['-- scope & policy --',''],
 ['Scope','Vendor code QZ73J / FC ISK3 only'],
 ['Cover policy','35d cap on ESTABLISHED SKUs only'],
 ['New arrivals','No cap - ship full ask (no DRR to cap on)'],
